...
`verify:data` re-derives each of these from the raw rows and compares them with
...
| Insight and narrative sentence templates | `lib/analytics/{insights,narrative}.ts` | SAFE — templates, every value interpolated from a query |
| Narrative thresholds | same | SAFE — tuning constants, not results |
...
| Chart colours, sizes, breakpoints | components | SAFE — UI sizing |
...
- **Evidence:** `signedCount` (`format.ts`) and `withOtherMarkets`
  ...
  slug = 'harborlight-inn'` — a no-op naming the old fictional property.
...
The narrative genuinely re-decides rather than re-rendering one conclusion:
...
The status line is range-aware, so a 30-day filter never produces "a strong
...
Labels state the actual comparison — "the same 30 days last year", "the 12
...
0.40–0.78s. No oversized images: the app ships no raster assets at all.
...
Calm: one accent, one serif-weighted display face for spoken lines, hairlines
...
oversell, which the seed refuses to write and the verifier re-derives from
...
| `reference-dashboard.png` | — | **Missing** |
