import { EmptyState } from "@/components/ui/EmptyState";
import { CAMPAIGN_COPY } from "@/lib/analytics/campaign-copy";
import { compactCurrency, count, currency, percent } from "@/lib/analytics/format";
import type { CampaignBreakdown } from "@/types/analytics";

/**
 * One strategy is usually carrying the period. Giving all four equal visual
 * weight hides that, so the leader gets the space and the rest form a quiet
 * ranked list beneath it. Which one leads is decided by revenue, not by type.
 */
export function LeadingStrategy({
  campaigns,
  propertyName,
}: {
  campaigns: CampaignBreakdown[];
  propertyName: string;
}) {
  const earning = campaigns.filter((c) => c.booking_revenue > 0);

  if (earning.length === 0) {
    return (
      <section>
        <h2 className="text-[22px] font-semibold tracking-[-0.015em] text-ink sm:text-[24px]">
          What drove your bookings
        </h2>
        <div className="mt-6">
          <EmptyState title="No campaign produced an attributed booking in this period." />
        </div>
      </section>
    );
  }

  const [leader, ...rest] = earning;
  const leadCopy = CAMPAIGN_COPY[leader.campaign_type];
  const maxRestShare = Math.max(...rest.map((c) => c.revenueShare), 0.0001);

  const heading =
    leader.campaign_type === "brand_protection"
      ? `Guests already looking for ${propertyName} drove the most revenue`
      : `Guests ${leadCopy.guestLabel.toLowerCase()} drove the most revenue`;

  return (
    <section>
      <h2 className="max-w-[22ch] text-[24px] font-semibold leading-[1.2] tracking-[-0.015em] text-ink sm:max-w-none sm:text-[28px]">
        {heading}
      </h2>

      <div className="mt-7 border-l-2 border-harbor pl-6">
        <p className="text-[13px] text-ink-faint">{leadCopy.label}</p>
        <p className="tnum mt-2 text-[36px] font-semibold leading-none tracking-[-0.025em] text-ink sm:text-[42px]">
          {currency(leader.booking_revenue)}
        </p>
        <p className="tnum mt-2.5 text-[15px] text-ink-soft">
          {count(leader.bookings)} direct bookings ·{" "}
          {percent(leader.revenueShare)} of booking revenue
        </p>
        <p className="mt-3 max-w-prose text-[15px] leading-relaxed text-ink-soft">
          {leadCopy.description}
        </p>
      </div>

      {rest.length > 0 ? (
        <div className="mt-9">
          <h3 className="eyebrow mb-4">Your other booking drivers</h3>
          <ul>
            {rest.map((campaign) => {
              const copy = CAMPAIGN_COPY[campaign.campaign_type];
              return (
                <li
                  key={campaign.campaign_id}
                  className="border-b border-rule/60 py-3.5 last:border-0"
                >
                  <div className="flex items-baseline justify-between gap-4">
                    <span className="text-[15px] text-ink">
                      {copy.guestLabel}
                      <span className="ml-2 text-[13px] text-ink-faint">
                        {copy.label}
                      </span>
                    </span>
                    <span className="tnum shrink-0 text-[15px] text-ink">
                      <span className="font-semibold">
                        {percent(campaign.revenueShare)}
                      </span>
                      <span className="ml-3 text-ink-soft">
                        {compactCurrency(campaign.booking_revenue)}
                      </span>
                    </span>
                  </div>
                  <div className="mt-2 h-1 w-full overflow-hidden rounded-full bg-surface-sunk">
                    <div
                      className="h-full rounded-full bg-harbor/45"
                      style={{
                        width: `${(campaign.revenueShare / maxRestShare) * 100}%`,
                      }}
                    />
                  </div>
                </li>
              );
            })}
          </ul>
        </div>
      ) : null}
    </section>
  );
}
